...
When the expression evaluates to `true` for a `hidden` attribute, the field is **visible**. When the expression evaluates to `false`, the field is **hidden**. This means that you should write the condition for hiding an item, not showing it.

Example: If you want to show a question only when "Do you like fruit? (Yes/No)" is answered with "Yes", you would drag the question `Do you like fruit?` into the expression field, then enter `==`, then drag the option `Yes` to the left of the expression.
